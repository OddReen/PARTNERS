...
public class UIAutoAnimation : MonoBehaviour
{
    ...
    private void AutoTriggerCalculation(SOAnimationPresets animationPresets)
    {
        ...
        {
            Invoke(nameof(ExitAnimation), maxEntranceDuration + animationPresets.autoTriggerTimer);
        }
        if (animationPresets.autoTrigger == SOAnimationPresets.AutoTrigger.TriggerEntranceAnimation)
        {
            Invoke(nameof(EntranceAnimation), maxEntranceDuration + animationPresets.autoTriggerTimer);
        }
        if (animationPresets.autoTrigger == SOAnimationPresets.AutoTrigger.Deactivate)
        {
            Invoke(nameof(DeactivateExitingMenu), maxEntranceDuration + animationPresets.autoTriggerTimer);
        }
        if (animationPresets.autoTrigger == SOAnimationPresets.AutoTrigger.Destroy)
        {
            Invoke(nameof(Destroy), maxEntranceDuration + animationPresets.autoTriggerTimer);
        }
    }
    private void DeactivateExitingMenu()
    {
        Menu.Instance.DeactivateExitingMenu();
    }
